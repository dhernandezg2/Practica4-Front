...
export const handler: Handlers = {
  async GET(_req, ctx) {
    try {
      const res = await fetch(`${API_BASE_URL}/api/posts/${ctx.params.id}`);
      if (!res.ok) throw new Error("Error al obtener post");
      const data: ApiResponseSingleSuccess<Post> = await res.json();
        ...
        <footer className="post-footer">
          <div className="post-actions">
            <LikeButton
              ...
              isLiked={false}
            />
             <span><DeleteButton postId={post._id} /></span>
          </div>
